...
# 2017/10/27 - u.alonsocamaro@f5.com - Renamed ansible-create to ansible-playbook
# 2017/10/27 - u.alonsocamaro@f5.com - Implemented ansible-role
# 2017/11/27 - u.alonsocamaro@f5.com - Introduction of attribute @json(list) to allow handling of JSON lists by tmsh2iapp by transforming the JSONlist into a space separated list.
#
...
#
# 2017/11/27 - u.alonsocamaro@f5.com - Added iapp_implementation_variables_log to print all variables when the iApp is being instantiated

$tmsh2iapp_version= "20171127.2";
...
$content= join("\n", grep(!/^\s*(@(label|apl|properties|iapp|import|json))/, split(/\n/, $raw_content)));
...
foreach $al (@attribute_lines) {
    ...
    if (/@(label|apl|properties|iapp|import|json)\((.*?)\):\s*(.*)/) {
	$attribute= $1;
	$variable= $2;
	$value= $3;
...
sub iapp_implementation_variables_log {
    ...
    $retval.= "puts \"Dumping values passed to iApp variables\"\n";

    $retval.= "puts \">>> regular variables\"\n";
    ...
    $retval.= "puts \">>> pool member variables\"\n";

    foreach $pm (sort @pool_members) {

        $vname= $pm;
        $vname=~ s/__(.*)__/$1/;
        $retval.= "puts \"> $pm >\${::$vname}<\"\n";

	$vname.= "_properties";
        $retval.= "puts \"> $pm properties >\${::$vname}<\"\n";
    }

    $retval.= "puts \">>> data record variables\"\n";

    foreach $dr (sort @data_records) {

        $vname= $dr;
        $vname=~ s/__(.*)__/$1/;
        $retval.= "puts \"> $dr >\${::$vname}<\"\n";
    }
    ...
    foreach $fwal (sort @fw_address_list) {
        ...
        $vname= $fwal;
        $vname=~ s/__(.*)__/$1/;
        $retval.= "puts \"> $fwal >\${::$vname}<\"\n";
    }

    $retval.= "puts \">>> firewall port list variables\"\n";

    foreach $fwpl (sort @fw_port_list) {

        $vname= $fwpl;
        $vname=~ s/__(.*)__/$1/;
        $retval.= "puts \"> $fwpl >\${::$vname}<\"\n";
    }

    $retval.= "puts \">>> url match category list variables\"\n";

    foreach $urlcat (sort @urlcat_match_list) {

        $vname= $urlcat;
        $vname=~ s/__(.*)__/$1/;
        $retval.= "puts \"> $urlcat >\${::$vname}<\"\n";
    }

    $retval.= "puts \">>> url nomatch category list variables\"\n";
    ...
    $retval.= "puts \">>> iworkflow variables\"\n";

    foreach $iwork (sort @iworkflow_variables) {

        $vname= $iwork;
        $vname=~ s/__(.*)__/$1/;
        $retval.= "puts \"> $iwork >\${::$vname}<\"\n";
    }

    $retval.= "puts \"End of dumping values passed to iApp variables\"\n";
    return $retval;
}
